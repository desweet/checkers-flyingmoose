...
public class Checker extends JPanel {
	...
	public Checker(Checker c, boolean select)
	{
		this.pos = new coordinate(c.pos.getX(),c.pos.getY());
		this.color = c.color;
		this.king = c.king;
		if (!select)
		{
		...
		else
		{
			if (c.equals(Color.RED)){
				if (king)
					checker = new ImageIcon("crownred_select.gif", "a red king");
				else
					checker = new ImageIcon("checkred_select.gif", "a red checker");
			}else{
				if (king)
					checker = new ImageIcon("crownblack_select.gif", "a black king");
				else
					checker = new ImageIcon("checkblack_select.gif", "a black checker");
			}
		}
